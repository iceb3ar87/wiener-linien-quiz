...
tram_endstations = list(tram_lines.values())

for line, correct_answer in tram_lines.items():
    options = random.sample(tram_endstations, 2) + [correct_answer]
...
ubahn_endstations = list(ubahn_lines.values())

for line, correct_answer in ubahn_lines.items():
    options = random.sample(ubahn_endstations, 2) + [correct_answer]
    random.shuffle(options)
...
district_names = list(districts.values())

for num, correct_answer in districts.items():
    options = random.sample(district_names, 2) + [correct_answer]
    random.shuffle(options)
